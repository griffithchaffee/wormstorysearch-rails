class LocationSearcher::FanfictionSearcher::Test < ApplicationTestCase
  ...
  testing "stories" do
    ...
    assert_equal(16, fanfiction_story.chapters.count)
    chapter = fanfiction_story.chapters.first
    assert_equal(
      {
        "category"=>"chapter",
        "chapter_created_on"=>fanfiction_story.story_updated_at.to_date.to_s,
        "chapter_updated_at"=>fanfiction_story.story_updated_at.to_s,
        "location_path"=>"/s/12337359/1/Cage",
        "position"=>"1",
        "title"=>"Cygnet 1-1",
        "word_count"=>"0"
  ...
  testing "crossovers" do
    ...
    chapters_html = SiteCrawler::HtmlParser.parse(chapters_html)
    searcher.update_chapters_for_story_from_html!(fanfiction_story, chapters_html)
    assert_equal(19, fanfiction_story.chapters.count)
    chapter = fanfiction_story.chapters.first
    assert_equal(
      {
        "category"=>"chapter",
