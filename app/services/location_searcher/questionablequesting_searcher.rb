module LocationSearcher
  class QuestionablequestingSearcher < UniversalSearcher
    attr_reader :config, :search_options, :story_model

    def initialize
      @story_model = QuestionablequestingStory
    ...
    def update_stories!(*params)
      update_sfw_stories!(*params)
      update_nsfw_stories!(*params)
    end

    def update_sfw_stories!(options = {})
    ...
    def update_nsfw_stories!(options = {})
      options = options.with_indifferent_access
      page = options.delete(:page) || 0
      # crawl "Creative Writing" subforum "Worm"
      loop do
    ...
    def update_quests!(*params)
      update_sfw_quests!(*params)
      update_nsfw_quests!(*params)
    end

    def update_sfw_quests!(options = {})
      options = options.with_indifferent_access
      page = options.delete(:page) || 0
    ...
    def update_nsfw_quests!(options = {})
      options = options.with_indifferent_access
      page = options.delete(:page) || 0
      # crawl "Roleplaying & Quests" forum
      loop do
    ...
    def update_stories_from_html!(html, options = {})
      options = options.with_indifferent_access.assert_valid_keys(*%w[ active_after attributes chapters ])
