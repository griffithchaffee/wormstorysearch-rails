  ...
  class SpacebattlesSearcher < UniversalSearcher
    ...
    def update_stories_newer_than!(time)
      ...
      while page do
        ...
        crawler.get("/forums/worm.115/#{"page-#{page}" if page > 1}", { order: "last_post_date", direction: "desc" }, { log_level: Logger::WARN })
        threads_html = crawler.html.find_all("ol.discussionListItems li.discussionListItem:not(.sticky)")
        # stop on last page
        break if threads_html.size == 0
        # parse threads
        threads_html.each do |thread_html|
          ...
          break if story.story_updated_at < time
        end
      end
    end
    ...
    def update_story_for_thread!(thread_html)
      ...
      updated_html = thread_html.css(".lastPostInfo .DateTime").first
      # build story
      ...
      story.assign_attributes(
        location_url: "/#{title_html[:href].remove(/\/(unread)?\z/)}",
        title: title_html.text,
        author: author_html.text,
        word_count: convert_word_count_to_i(word_count_html.text.remove("Word Count: ")),
        story_created_on: created_html["data-time"] ? Time.at(created_html["data-time"].to_i).to_date : Date.parse(created_html.text),
        story_updated_at: created_html["data-time"] ? Time.at(created_html["data-time"].to_i) : Time.parse(created_html.text),
      )
    ...
    def update_chapters_for_story!(story)
      # get threadmarks
      crawler.get("#{story.location_url}/threadmarks", {}, { log_level: Logger::WARN })
      # parse threadmarks
      position = 0
      crawler.html.find_all("li.primaryContent").each do |html_li|
        ...
        # build chapter
        location_url = preview_html[:href]
        chapter = story.chapters.get(position: position) || story.chapters.build(position: position)
        keep_created_at = chapter.saved? && chapter.location_url == preview_html[:href]
